...
def get_top_kosher_fund():
    # גלישה לדף ריכוז הקרנות הכספיות ב-Funder - אתר אמין ומעודכן
    ...
    try:
        ...
        # בדרך כלל הטבלה המרכזית היא הטבלה הראשונה או השנייה
        df = tables[0] if len(tables[0]) > 10 else tables[1]
        
        # ניקוי שמות עמודות - הסרת רווחים ותווים מוזרים
        df.columns = [str(c).strip() for c in df.columns]
        
        # איתור עמודות (לפי מילות מפתח בטבלה של Funder)
        ...
        if kosher_df.empty:
            print("No kosher funds found in table.")
            return None

        # פונקציה לניקוי אחוזים והפיכה למספר
        def clean_num(val):
        ...
        # נוסחת נטו: תשואה יומית פחות חלק יחסי של דמי ניהול (שנתי חלקי 365)
